...
3. **A covariate-shift and taxonomy-confounding result.** The gradient is strong within taxonomic distribution and weakens at family level, while a taxonomy-majority baseline reveals that pathogenicity is heavily clade-confounded (§4.3, §4.9). We further organize all 21 traits into a four-way failure-mode taxonomy that localizes the cross-clade wall to 4 coverage-limited traits and shows it shares one neighbour-coverage mechanism (§4.6--4.7), then test the three implied remedies---broader clade coverage, a stronger encoder, and label-schema fixes---and quantify how much each recovers (§4.8). Probing the collapse, we further find it is training-clade-coverage-limited rather than a representation wall, and a label-free retrieval blend recovers a small but consistent part of the lost cross-clade transfer without fine-tuning (§4.5).
...
## The collapse is covariate shift, not overfitting

A monotone cross-clade decay could in principle reflect ordinary overfitting---a model that memorizes its training genomes and degrades on any held-out data---rather than a distribution-shift effect specific to novel clades. The two are separable by their *train-versus-test* signature read across the split hierarchy: overfitting produces a large train--test gap already on the easiest (species) split, whereas a covariate-shift limit produces a small in-distribution gap that widens as the held-out clades grow more distant. We train the encoder MLP and per-trait heads on mean-pooled ESM-2 features under each split and evaluate the same model on its own training set and on held-out test clades (3 seeds, 40 epochs; Table 27).

The signature is unambiguously the latter. The macro train--test gap is only $+0.014$ on the species split, and *training* macro-F1 is essentially flat across splits ($0.678 / 0.689 / 0.683$); it is the *test* score that falls ($0.665 \to 0.658 \to 0.647$), so the gap widens monotonically with taxonomic distance ($+0.014 \to +0.030 \to +0.036$). Seed dispersion is negligible ($\pm 0.001$--$0.004$). A memorizing model would already show a large species-split gap and seed sensitivity; neither is present. With a frozen encoder and a small, weight-decayed head trained on only $\sim$9--12k genomes there is little capacity to memorize, and the data confirm it: the aggregate failure is an out-of-distribution coverage limit, not variance.

The per-head decomposition refines this into two distinct, separable failure types. A handful of *sparse, high-cardinality* heads do overfit in the classical sense---`amr_phenotype` carries the same large gap on species and family ($+0.13$ in both), the fingerprint of label starvation ($\sim$6.9k labels) independent of clade. A second group shows the covariate-shift signature instead: `cultivation_medium`'s gap grows from $+0.034$ (species) to $+0.169$ (family) and `motility`'s from $+0.049$ to $+0.118$---they generalize in-distribution but fail to extrapolate to unseen clades that bring novel media and phenotypes. Well-supported machinery heads (catalase, cell shape, gram stain) have small, flat gaps throughout. The localized overfitting is therefore a property of specific data-starved labels---addressable only with more labelled positives---while the dominant, model-level effect remains taxonomic coverage. This corroborates, directly from the train--test gap, the conclusion the retrieval (§4.5) and coverage-scaling (§4.8) analyses reach from the opposite direction.

![**Train versus held-out test across the cross-clade split hierarchy (mean-pool ESM-2, 3 seeds).** Training macro-F1 is flat across splits while held-out test falls, so the train--test gap widens monotonically with taxonomic distance ($+0.014$ species $\to +0.036$ family). The small in-distribution gap rules out overfitting as the aggregate failure mode: the cross-clade collapse is an out-of-distribution coverage limit, not variance.](figures/27_overfit_gap.png){width=85%}
...
## Not every trait fails the same way

"The gradient collapses at family holdout" is a statement about the *machinery* traits; it is not the whole story for the 21-trait panel. To avoid over-generalizing from the binary traits we validate in depth, we classify every trait by two coordinates: its in-distribution ceiling (species-split macro-F1) and its cross-clade decay (species-split minus family-split macro-F1). This yields four qualitatively different regimes (Table 20, Figure 6). **Solved** traits (4: catalase, cytochrome oxidase, pigmentation, sporulation) reach family-split F1 $\geq 0.70$ and barely decay---these are localized, redox- or sporulation-machinery traits whose signal transfers across clades. **Label-ceiling** traits (6: e.g. `country` 0.020, `metabolite_production` 0.102, `cell_shape` 0.190 species-F1) are poor *even in-distribution*; the failure is in the label or the task definition, not the model or the embedding, and no pooling or retrieval change will help them. **Moderate-flat** traits (6: e.g. `gram_stain`, `ph_class`, `carbon_utilization`) carry partial signal that neither collapses nor saturates. The interesting middle is the **coverage-limited** set (4: `pathogenicity_animal` decay 0.319, `pathogenicity_human` 0.270, `temperature_class` 0.181, `motility` 0.177): learnable in-distribution but collapsing off-clade. This is the only regime where the §4.5 retrieval diagnosis applies, and it is the regime the rest of this section dissects. Crucially, the taxonomy makes the negative result *specific*: the cross-clade wall is a property of 4 of 21 traits, not a blanket ceiling, and it co-locates exactly with the traits whose positives are clade-restricted.

![**Failure-mode taxonomy.** Each trait placed by in-distribution ceiling (species-split F1, x-axis) against cross-clade decay (species$-$family F1, y-axis). Coverage-limited traits (high ceiling, high decay) are the only ones the retrieval diagnosis targets; label-ceiling traits (low ceiling) are unsolved in-distribution and out of scope for pooling or retrieval.](figures/failure_modes.png){width=90%}

## One coverage mechanism, replicated across the coverage-limited traits

The §4.5 retrieval result establishes that cross-family signal *exists*; here we show *why* it is lost at the threshold and that the mechanism is shared, not idiosyncratic to one trait. For each binary coverage-limited trait we measure, on family-held-out test genomes, the recall on true positives stratified by how many of each genome's $k=10$ nearest *training-family* neighbours carry the label (Table 21, Figure 7). The pattern is identical in all three: ranking quality stays high while macro-F1 lags (AUROC$-$F1 gap $+0.18$ human, $+0.21$ animal, $+0.06$ motility), positives are heavily concentrated in a handful of training families (Gini $\geq 0.85$; top-5 families hold 40--44\% of all positives), and recall on novel-family positives rises monotonically with neighbour coverage---from $0.11$ to $0.72$ (human pathogenicity), $0.18$ to $0.61$ (animal), and $0.00$ to $0.63$ (motility) between the no-coverage and high-coverage bins. The collapse is therefore not a representational ceiling but a *support* problem: a novel-family positive is recoverable exactly when the embedding neighbourhood it lands in was labelled positive in training. This is the mechanistic basis for both the §4.5 retrieval gain (it imports neighbour labels) and its modesty (it cannot manufacture coverage that the training clades never supplied), and it makes the prescription concrete: the lever for these four traits is broader training-clade sampling of the positive class, not a different pooling operator.

![**The coverage mechanism replicates.** Recall on novel-family true positives versus the fraction of $k$-nearest training neighbours that are positive, for the three binary coverage-limited traits. Recall rises monotonically with neighbour coverage in every case; cross-clade recall tracks support, not model capacity.](figures/coverage_panel.png){width=100%}

## Which levers actually move the needle

The taxonomy and the coverage mechanism predict *where* headroom lives; we test the three implied levers directly, all on frozen features with transparent probes (no encoder fine-tuning), and find they help exactly where the diagnosis says they should.

**Lever 1 --- broader training-clade coverage (Table 22, Figure 8).** For each coverage-limited trait we trace family-held-out performance against the number of distinct training families. Growing clade coverage lifts cross-clade transfer substantially: family-test AUROC rises $0.62\to0.76$ (human pathogenicity), $0.59\to0.77$ (animal), and $0.53\to0.70$ (motility) as training families grow from a handful to the full set, i.e. $+0.02$--$0.035$ AUROC per doubling of training-clade coverage. A size-controlled variant that pins the genome budget to the smallest-$k$ pool is underpowered for these rare-positive traits (the budget collapses to 9--32 genomes) and shows no diversity effect *beyond* volume, so we report the actionable natural curve and are explicit that, at these positive counts, we cannot cleanly separate "more clades" from "more positives"---both arrive together when you sample more broadly, and both are the recommendation.

**Lever 2 --- a stronger encoder (Table 23, Figure 9).** Holding the family split, probe, and the 12,062 shared genomes fixed and swapping only the frozen representation---the 150M ESM-2 protein LM for Bacformer, a bacterial-genome foundation model---gives a small but mostly positive lift (mean $\Delta$ family-test AUROC $+0.012$, $\Delta$F1 $+0.013$ across seven traits), concentrated in the localized traits (sporulation $+0.021$ AUROC, pigmentation $+0.053$, human-pathogenicity F1 $+0.056$). The effect is modest but consistent and is direct evidence that encoder quality is a live lever the pooling and retrieval experiments leave untouched; it motivates re-extracting features from a larger ESM-2 (650M/3B), which our extraction scripts support on GPU.

**Lever 3 --- fix the labels, not the model (Table 24).** The six label-ceiling traits fail for heterogeneous reasons we can now name: two are *structured/multilabel* fields (`metabolite_production`, `cultivation_medium`) collapsed into a single label; one is *geography* (`country`, 246 classes, no dominant class) with no genomic basis; `isolation_source` is a *free-text explosion* (24k strings, 80\% singletons); and `cell_shape`/`oxygen_tolerance` are few-class but noisy. Consolidating the schema and re-probing in-distribution confirms the diagnosis is actionable where the pathology is schema, not signal: collapsing `oxygen_tolerance` from six noisy classes to three canonical ones ({aerobe, anaerobe, facultative}) raises species-split macro-F1 $0.343\to0.420$ ($+0.078$), while canonicalizing `isolation_source` into eight ecological categories makes the task well-defined but barely moves F1 ($+0.003$)---its ceiling is genuine weak signal, and `country` is simply out of genomic scope. The lesson is that "improve the model" is the wrong response to a third of the panel; the right responses are a multilabel reformulation, a schema cleanup, or dropping the trait.

We then carry out the multilabel reformulation for the two structured traits (Table 25) and confirm the diagnosis directly. Posing `metabolite_production` as a masked binary task per compound (train and score each compound only on the genomes actually tested for it) lifts species-split macro-F1 from the $0.102$ single-label collapse to $0.405$ (macro-AUROC $0.603$), with individual compounds such as nitrite (AUROC $0.79$) and indole ($0.67$) clearly learnable. Posing `cultivation_medium` as multilabel presence per medium is more striking: macro-AUROC reaches $0.891$ over the 54 well-supported media (collapse macro-F1 $0.278 \to$ micro-F1 $0.424$), with many individual media predicted near-perfectly (e.g. media 11, 645, 585 at AUROC $\geq 0.99$). The frozen representation clearly carries this signal; only the single-label encoding hid it.

For `cultivation_medium` we close the loop from rank quality to usable predictions (Table 26), which also exposes what was suppressing its thresholded F1. Per-medium decision-threshold tuning on a validation split barely moves macro-F1 ($0.382 \to 0.378$): the problem is not the threshold. The labels are positive-only, and an Elkan--Noto SCAR estimate puts the labeling propensity at $c \approx 0.53$ --- only about half of the genomes that actually grow on a medium are recorded as such --- so naive precision is penalizing correct predictions of true-but-unlisted growth. Correcting for this (crediting each unlabeled example by its estimated posterior of being a hidden positive) lifts macro-F1 to $0.640$, with well-sampled media reaching $0.85$--$0.90$ (e.g. media 65, 11, 84). The honest caveat is that this PU-adjusted F1 uses the model's own scores to credit the corrections and is therefore partly model-dependent and likely optimistic; but the independently-estimated $c$ shows the underlying issue is real label incompleteness, not a modelling failure. Two of the six "unsolved" traits, then, were never a representational ceiling at all --- one (`metabolite_production`) needed the right multi-label formulation, and the other (`cultivation_medium`) needed that plus an explicit account of positive-only labels.

![**Lever 1: coverage scaling.** Family-held-out macro-F1 (left) and AUROC (right) rise with the number of training families for all three coverage-limited traits.](figures/coverage_scaling.png){width=100%}

![**Lever 2: encoder swap.** Family-held-out AUROC per trait for ESM-2 (150M) vs Bacformer on the shared genome set; the stronger encoder gives a small consistent lift on the localized traits.](figures/encoder_comparison.png){width=90%}
...
- *Covariate shift is unsolved and dominates.* The benefit evaporates at family-level holdout (§4.3). For the uncultured-organism application that motivates genomic trait models, this is the result that matters most, and it is negative for the pooling lever. It is at least *detectable*: a label-free embedding monitor flags novel-family genomes at AUROC 0.76, though that signal does not transfer to per-genome confidence. Probe-level analysis (§4.5) further shows the collapse is clade-coverage-limited rather than a representation wall, and a retrieval blend recovers a small, consistent part of the gap. We then quantify the three implied levers (§4.8): broader training-clade coverage is the largest, lifting family-test AUROC by up to $+0.18$ for pathogenicity; a stronger encoder (Bacformer vs 150M ESM-2) adds a small consistent $+0.012$ mean AUROC; and a third of the unsolved traits are label-schema artifacts, not modelling failures --- consolidating `oxygen_tolerance` to three classes adds $+0.078$ macro-F1, and reformulating the two structured traits as multi-label recovers large signal the single-label collapse hid (`metabolite_production` macro-F1 $0.405$ vs collapse $0.102$; `cultivation_medium` macro-AUROC $0.891$, and after a positive-unlabeled correction a usable macro-F1 $0.640$ vs collapse $0.278$). None of these is a different pooling operator.
...
The repository includes the trait schema, label matrix, split definitions, model code, pooling run summaries, VFDB attribution artifacts, and the analysis generator used for the new reviewer-facing tables. The generated audit artifacts cover absolute pooling results, the preliminary localization audit, the taxonomy-majority baseline, the cross-clade transfer analyses of §4.5 (`cross_clade_diagnostic.py`, `retrieval_head.py`, `adaptive_retrieval.py`, regenerating Tables 15--17), the failure-mode taxonomy and coverage-mechanism analyses of §4.6--4.7 (`paper/failure_mode_analysis.py`, `trait_coverage_analysis.py`, `paper/coverage_panel.py`, regenerating Tables 20--21 and the corresponding figures), and the three lever experiments of §4.8 (`paper/coverage_scaling.py`, `paper/encoder_comparison.py`, `paper/label_quality_audit.py`, `paper/multilabel_reformulation.py`, `paper/cultivation_medium_pu.py`, regenerating Tables 22--26 and Figures 8--9). Large protein embedding stores and trained checkpoints are not bundled in this manuscript artifact, but the scripts and identifiers needed to regenerate them are included.
