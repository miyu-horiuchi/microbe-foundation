...
# DDP takes precedence when requested, since it already consumes all the GPUs for
# the single heavy job.
NPROC="${NPROC:-${GPUS_PER_NODE:-}}"
DDP_ON=0
case "$NPROC" in
  ""|0|1) DDP_ON=0 ;;
  auto)   [ "${GPU_COUNT:-1}" -gt 1 ] 2>/dev/null && DDP_ON=1 ;;
  *)      [ "$NPROC" -gt 1 ] 2>/dev/null && DDP_ON=1 ;;
esac
